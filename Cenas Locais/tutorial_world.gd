...
# ==========================================
# NÓS DO TUTORIAL (Substitua os caminhos arrastando da sua cena!)
# ==========================================
...
@onready var botao_iniciar_noite = $CanvasLayer/BotaoNoite 
...
func iniciar_sequencia_tutorial():
	...
	var botao_torre1 = slot_torre_1.ui_atual._botoes_ativos[0] 
	...
	await get_tree().create_timer(0.2).timeout 
	var botao_torre2 = slot_torre_2.ui_atual._botoes_ativos[0]  
	...
	await get_tree().create_timer(0.2).timeout
	var botao_casa1 = slot_casa_1.ui_atual._botoes_ativos[1]
	...
	await get_tree().create_timer(0.2).timeout
	var botao_casa2 = slot_casa_2.ui_atual._botoes_ativos[1] 
	await tutorial.focar_em_ui_2d(botao_casa2, "Afonso: Excelente. Ouro garantido para o pequeno-almoço.")
	await get_tree().create_timer(1.0).timeout

	# --- DIA 1: INICIAR A NOITE (BUSCANDO O BOTÃO DA HUD) ---
	# O Godot vai procurar no jogo inteiro quem tem o crachá "BotaoIniciarNoite"
	...
	var botao_upgrade_castelo = castelo.ui_atual.get_node("Upgrade") 
	...
	var botao_quartel = slot_quartel.ui_atual.get_node("Quartel") 
	...
	# --- FIM DO TUTORIAL ---
	# Procura o botão de iniciar a noite novamente para garantir que ele é o atual
